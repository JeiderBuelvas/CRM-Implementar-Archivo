...
if (!class_exists('TCPDF', false)) {
	...
	*/
	class TCPDF {
		...
		protected function setFooter() {
			...
			if ($this->print_footer) {
				...
				if (isset($this->footer_font) && is_array($this->footer_font) && count($this->footer_font) >= 3) {
                 // Verificar que $this->footer_font esté definido, sea un array y tenga al menos 3 elementos
                $this->SetFont($this->footer_font[0], $this->footer_font[1], $this->footer_font[2]);
                } else {
                // En caso de que $this->footer_font no esté definido o no sea un array válido, puedes establecer valores predeterminados
                // Por ejemplo:
                // $this->SetFont('Arial', '', 10); // Aquí puedes usar los valores que desees para la fuente
                }
        ...
        $version = PHP_VERSION;
        define('PHP_VERSION_ID', ((int)substr($version, 0, 1) * 10000 + (int)substr($version, 2, 1) * 100 + (int)substr($version, 4)));
         }
    if (PHP_VERSION_ID < 50300) {
		...
		public function get_mqr() {
			if(!defined('PHP_VERSION_ID')) {
				$version = PHP_VERSION;
				define('PHP_VERSION_ID', ((int)$version[0] * 10000 + (int)$version[2] * 100 + (int)$version[4]));
			...
			while ($elkey < $maxel) {
				...
				if (preg_match($tagpattern, $element)) {
					...
					if ($element[0] == '/') {
						...
						if (($dom[$key]['value'][0] == 'h') && (intval($dom[$key]['value'][1]) > 0) && (intval($dom[$key]['value'][1]) < 7)) {
								...
								if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_ORDER)) {
									...
									while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
										...
										if (($stroffset !== false) AND ($stroffset <= $strpiece[2][1])) {
											...
											while (($offset !== false) && ($pmid[($offse - 1)] == '\\')) {
